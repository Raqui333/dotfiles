...
^p(20)$([ -z "$(mpc current -f %artist%)" ] && echo Paused || mpc current -f %artist%)
^p(20)$([ -z "$(Music)" ] && echo Paused || echo $(Music))

^p(45)^ca(1,mpc prev)^i($icon/Prev.xbm)^ca()   ^ca(1,mpc play)^i($icon/Play.xbm)^ca()   ^ca(1,mpc stop)^i($icon/Stop.xbm)^ca()  ^ca(1,mpc next)^i($icon/Next.xbm)^ca()
